...
namespace GridExemplo
{
    public class MeuFormGrid
    {
        ...
        private SAPbouiCOM.StaticText oStaticHeader;
        private SAPbouiCOM.StaticText oStaticHead;
        private SAPbouiCOM.StaticText oStaticGroup;
        private SAPbouiCOM.Button oBtnCo;
        ...
        private void ColorirLinhas()
        {
            ...
            {
                oGrid.CommonSetting.SetRowFontStyle(1 + 1, SAPbouiCOM.BoFontStyle.fs_Italic);
                if ((i + 1) % 2 == 0)
                {
                    //verde
            ...
            if (FormUID.Equals("frmGrid"))
            {
                if (pVal.ItemUID.Equals("optNo") | pVal.ItemUID.Equals("optCode") | pVal.ItemUID.Equals("optDate") | pVal.ItemUID.Equals("optStatus"))
                {
                    ...
                    else if (userDS.Value.Equals("4"))
                    {
                        ...
                        oForm.Freeze(true);
                        oForm.DataSources.DataTables.Item(0).ExecuteQuery("select DocStatus, DocDate, DocNum, DocTotal , DocTotal from OINV");
                        oGrid.CollapseLevel = 1;
                        AtualizarTiposColunas();
                        ColorirLinhas();
                        oForm.Freeze(false);
